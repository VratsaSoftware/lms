...
<div class="row g-0 home-work-1 align-items-center p-3 mt-3">
    <div class="col-auto mx-lg-0 mx-auto d-lg-none lection-eval">
        <button style="margin-top: -50px!important;" onclick="window.location.href='{{ route('homework.comments', encrypt($myHomework->id)) }}'" class="ms-xxl-2 mt-xxl-0 mt-4 btn-view-1 btn-green row g-0 align-items-center">
            <div class="col-auto mx-auto fw-bold see-all">Виж коментари ({{ $myHomework ? $myHomework->Comments->where('is_valid', 1)->count() : 0 }})</div>
        </button>
    </div>
    <!--END mobil-->
    <a href="{{ route('homework.comments', encrypt($myHomework->id)) }}" style="margin-left: -40px;" class="col-5 ps-5 text-normal text-white d-none d-lg-block">
        <span class="text-uppercase">Коментари</span> <span style="color: #8de219;">({{ $myHomework ? $myHomework->Comments->where('is_valid', 1)->count() : 0 }})</span>
